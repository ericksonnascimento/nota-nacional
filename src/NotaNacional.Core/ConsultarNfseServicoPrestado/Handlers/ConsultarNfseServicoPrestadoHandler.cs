    ...
    public class ConsultarNfseServicoPrestadoHandler : BaseHandler, IConsultarNfseServicoPrestadoHandler
    {
        ...
        public BaseResponse Handle(object header, object body, string ipUsuario, string cpfCnpjCertificado = "")
        {
            ...
            try
            {
                ...
                if (erros.Length != 0)
                {
                    var result = _repository.Find(Util.XmlVazio(OperacaoNfse.ConsultarNfseServicoPrestado), string.Empty, erros, ipUsuario);
                    return BuildResponse(result);
                }

                if (_apenasValidar)
                {
                    var result = _repository.Find(Util.XmlVazio(OperacaoNfse.ConsultarNfseServicoPrestado), string.Empty, string.Empty, ipUsuario);
                    return BuildResponse(result);
                }
            ...
            {
                var result = _repository.Find(Util.XmlVazio(OperacaoNfse.ConsultarNfseServicoPrestado), string.Empty, "E232", ipUsuario); //Ocorreu um erro no processamento do arquivo.
                return BuildResponse(result);
            }
        }
