...
namespace NotaNacional.Core.ConsultarNfseDps.Handlers
{

    public class ConsultarNfseDpsHandler : BaseHandler, IConsultarNfseDpsHandler
    {
        ...
        private readonly IConsultarNfseDpsRepository _repository;
        private readonly bool _apenasValidar;

        public ConsultarNfseDpsHandler(ICabecalhoValidator cabecalhoValidator,
            IConsultarNfseDpsValidator consultarNfseDpsValidator,
            IConsultarNfseDpsRepository repository,
        ...
        public BaseResponse Handle(object header, object body, string ipUsuario, string cpfCnpjCertificado = "")
        {
            ...
            try
            {
                ...
                ////Validar corpo
                var bodyValidator = _consultarNfseDpsValidator.Validate(body);
                ...
                if (erros.Length != 0)
                {
                    var result = _repository.Find(Util.XmlVazio(OperacaoNfse.ConsultarNfseDps), string.Empty, erros, ipUsuario);
                    return BuildResponse(result);
                }

                if (_apenasValidar)
                {
                    var result = _repository.Find(Util.XmlVazio(OperacaoNfse.ConsultarNfseDps), string.Empty, string.Empty, ipUsuario);
                    return BuildResponse(result);
                }
            ...
            {
                var result = _repository.Find(Util.XmlVazio(OperacaoNfse.ConsultarNfseDps), string.Empty, "E232", ipUsuario); //Ocorreu um erro no processamento do arquivo.
                return BuildResponse(result);
            }
        }

        private NotaNacional.Core.Models.ConsultarNfseDpsResposta BuildResponse(WsNfseConsultarNfseDpsResult result)
        {
